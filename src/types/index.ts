...
export interface Property {
  ...
  reviews?: { id: string; name: string; rating: number; date: string; comment: string }[];
}
...
export type PaymentMethod = 'raast' | 'bank_transfer' | 'rapidpaisa' | 'stripe' | 'easypaisa' | 'jazzcash' | 'cash';
export type BookingStatus = 'pending' | 'confirmed' | 'completed' | 'cancelled';
export type PaymentStatus = 'pending' | 'paid' | 'escrow_held' | 'released' | 'refunded' | 'release_requested' | 'raast_pending_verification';
...
export interface Booking {
  ...
  raastRrn?: string;
  raastEscrowRef?: string;
  raastPayerPhone?: string;
  raastSlipUrl?: string;
  raastVerifiedAt?: string;
  notes?: string;
  createdAt: string;
}
...
export interface BankDetails {
  ...
  accountTitle: string;
  iban: string;
  raastId: string; // e.g. 03182055632 (Official State Bank Raast ID)
  raastIban: string; // e.g. PK92MEZN0001020304050607
...
export type PaymentGateway = 'raast' | 'jazzcash' | 'easypaisa' | 'bank_transfer' | 'rapidgateway' | 'card';
