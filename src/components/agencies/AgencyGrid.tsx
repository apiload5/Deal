...
export const AgencyGrid: React.FC<AgencyGridProps> = ({ agencies, onSelectAgency }) => {
  return (
    <div className="space-y-6 my-6">
      ...
      {agencies.length === 0 ? (
        <div className="glass-card rounded-2xl p-10 text-center border border-slate-800/80 my-8 space-y-3 max-w-md mx-auto">
          <div className="w-14 h-14 bg-orange-500/10 border border-orange-500/20 rounded-2xl flex items-center justify-center mx-auto text-orange-400">
            <Building2 className="w-7 h-7" />
          </div>
          <h3 className="text-base font-bold text-white">No Corporate Agencies Registered Yet</h3>
          <p className="text-xs text-slate-400">
            Real estate agencies can register their office to post listings and manage agents under DealFast corporate escrow accounts.
          </p>
        </div>
      ) : (
