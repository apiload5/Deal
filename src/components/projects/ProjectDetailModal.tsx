          ...
          <div className="pt-4 border-t border-slate-800 space-y-3">
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 pt-2">
              <div className="flex items-center gap-2">
                <button
                  onClick={() => alert(`Downloading official Payment Plan & RDA/CDA NOC Brochure for ${project.title}`)}
                ...
                {onOpenVerificationPortal && (
                  <button
                    onClick={() => onOpenVerificationPortal('noc', project.title)}
                    className="bg-blue-600 hover:bg-blue-500 text-white border border-blue-500/50 px-3 py-2 rounded-xl text-xs font-bold flex items-center space-x-1.5 shadow-md"
                  >
                    <ShieldCheck className="w-3.5 h-3.5" />
                    <span>Verify Authority NOC</span>
                  </button>
                )}
              </div>

              <span className="text-[11px] text-emerald-400 font-bold flex items-center">
                <CheckCircle2 className="w-3.5 h-3.5 mr-1" /> RDA / CDA Approved Project
              </span>
            </div>
          </div>
        </div>
