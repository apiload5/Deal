...
export const UserProfileModal: React.FC<UserProfileModalProps> = ({
  ...
  const [avatar, setAvatar] = useState(user.avatar || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=200');
  ...
  React.useEffect(() => {
    if (isOpen) {
      ...
      setName(u.name || '');
      setAvatar(u.avatar || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=200');
      setPhone(u.phone || '');
      setEmail(u.email || '');
      ...
      <div className="bg-slate-900 border border-slate-700/80 w-full max-w-lg rounded-3xl overflow-hidden shadow-2xl my-auto relative text-xs">
        ...
        <div className="p-4 sm:p-5 bg-slate-950 border-b border-slate-800 flex items-center justify-between sticky top-0 z-30">
          <div className="flex items-center space-x-2 sm:space-x-3">
            ...
            <div>
              <h3 className="font-black text-white text-sm sm:text-base">Profile & Security Settings</h3>
              <p className="text-[11px] text-slate-400">Manage identity, privacy & CNIC verification</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSave} className="p-5 space-y-4 max-h-[75vh] overflow-y-auto">
          ...
          {/* CNIC Documents Scan Section */}
          <div className="p-3.5 rounded-2xl bg-slate-950 border border-slate-800 space-y-3">
            <h4 className="font-bold text-white text-xs flex items-center justify-between">
              <span className="flex items-center gap-1.5">
                <FileText className="w-4 h-4 text-orange-400" />
                <span>CNIC Document Scans (For Verification)</span>
              </span>
              <span className="text-[10px] text-slate-400 font-normal">256-Bit Encrypted Storage</span>
            </h4>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <ImageUpload
                label="Upload CNIC Front Image"
                multiple={false}
                onUploadComplete={urls => setCnicFrontUrl(urls[0] || '')}
              />
              <ImageUpload
                label="Upload CNIC Back Image"
                multiple={false}
                onUploadComplete={urls => setCnicBackUrl(urls[0] || '')}
              />
            </div>
            
            {(cnicFrontUrl || user.kycDocuments?.cnicFront) && (
              <p className="text-[10px] text-emerald-400 font-bold flex items-center gap-1">
                <CheckCircle2 className="w-3 h-3" />
                <span>CNIC Front & Back Scans Uploaded Safely</span>
              </p>
            )}
          </div>
