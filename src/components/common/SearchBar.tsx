      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
          ...
          <select
            ...
            className="min-h-[38px] bg-slate-950 border border-slate-800 rounded-xl px-2.5 sm:px-3 py-1.5 sm:py-2 text-[11px] sm:text-xs font-semibold text-slate-200 outline-none focus:border-orange-500 cursor-pointer"
          >
          ...
          {/* Sort By */}
          <div className="min-h-[38px] inline-flex items-center gap-1.5 bg-slate-950 border border-slate-800 rounded-xl px-2.5 sm:px-3 py-1.5 text-[11px] sm:text-xs text-slate-300">
            ...
            <select
              ...
              className="bg-transparent text-[11px] sm:text-xs font-semibold outline-none cursor-pointer text-slate-200"
            >
          ...
          <button
            ...
            className="min-h-[38px] inline-flex items-center justify-center gap-1.5 px-3 sm:px-3.5 py-1.5 sm:py-2 rounded-xl bg-orange-500/10 text-orange-400 border border-orange-500/30 text-[11px] sm:text-xs font-bold hover:bg-orange-500/20 transition-all leading-none whitespace-nowrap"
          >
          ...
          <button
            ...
            className="min-h-[38px] min-w-[38px] p-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-white transition-colors inline-flex items-center justify-center shrink-0"
            title="Reset Filters"
          >
        ...
        <div className="mt-4 pt-4 border-t border-slate-800 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 animate-in fade-in slide-in-from-top-2">
          ...
          <div>
            ...
            <input
              ...
              onChange={e => {
                ...
                handleUpdate({ ...filters, minPrice: raw === '' ? 0 : Number(raw) });
              }}
              className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs font-semibold text-slate-200 outline-none focus:border-orange-500 placeholder:text-slate-600"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              value={!filters.maxPrice || filters.maxPrice === 1000000000 ? '' : filters.maxPrice}
              onChange={e => {
                const raw = e.target.value.replace(/^0+(?=\d)/, '');
                handleUpdate({ ...filters, maxPrice: raw === '' ? 1000000000 : Number(raw) });
              }}
              className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs font-semibold text-slate-200 outline-none focus:border-orange-500 placeholder:text-slate-600"
            />
          </div>
            ...
            <div className="flex gap-1">
              {['any', 1, 2, 3, 4, 5].map(num => (
                <button
                  ...
                  className={`flex-1 min-h-[34px] py-1 px-1 rounded-lg text-[11px] sm:text-xs font-bold transition-all inline-flex items-center justify-center text-center leading-none ${
                    filters.beds === num
