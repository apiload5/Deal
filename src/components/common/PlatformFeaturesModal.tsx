...
import { X, ShieldCheck, Sparkles, Video, FileText, Building2, UserCheck, Calculator, Smartphone, MessageSquare, CheckCircle2, ArrowRight } from 'lucide-react';
...
export const PlatformFeaturesModal: React.FC<PlatformFeaturesModalProps> = ({ isOpen, onClose, onNavigateTab }) => {
  const [activeCategory, setActiveCategory] = useState<'all' | 'escrow' | 'ai' | 'agents' | 'projects'>('all');
  ...
  const features = [
    {
      ...
      description: 'Protects buyers from double allotment or fake sellers. 10% token deposit is securely locked in DealFast Escrow until RDA/CDA title verification and society NOC clearance.',
    ...
    },
    {
      id: 'ai-assistant',
      category: 'ai',
      title: 'AI Real Estate Advisor (Gemini 2.5 Flash Engine)',
      badge: 'AI Powered',
      icon: Sparkles,
      color: 'text-yellow-400 bg-yellow-500/10 border-yellow-500/30',
      description: 'Trained on Pakistani real estate laws, FBR 236K/236C tax rates, DHA/Bahria valuation tables, and society NOC status. Strictly handles property-related queries.',
      actionLabel: 'Ask AI Advisor',
      actionTab: 'ai'
    },
    {
      id: 'webrtc-calls',
    ...
    },
    {
      ...
      description: 'Agencies can post bounty tasks (e.g. PKR 3,000) for local agents in any city to physically inspect plots, take live site videos, and check RDA/CDA NOC records.',
    ...
    },
    {
      ...
      badge: 'Installment Plans & NOC',
      icon: Building2,
      color: 'text-purple-400 bg-purple-500/10 border-purple-500/30',
      description: 'Dedicated portals for mega projects like GSF, Emaar, and Capital Smart City featuring floor plans, Meezan/HBL Islamic home loan integrations, and downloadable payment PDFs.',
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-slate-950/85 backdrop-blur-md animate-in fade-in overflow-y-auto">
      <div className="bg-slate-900 border border-slate-700/80 w-full max-w-4xl rounded-3xl overflow-hidden shadow-2xl my-auto max-h-[90vh] flex flex-col">
        ...
        <div className="px-6 py-3 bg-slate-900/90 border-b border-slate-800 flex items-center space-x-2 overflow-x-auto shrink-0 no-scrollbar">
          {[
            { id: 'all', label: 'All Capabilities (8)' },
            { id: 'escrow', label: 'Escrow & Legal' },
            { id: 'ai', label: 'AI & Tax Advisors' },
