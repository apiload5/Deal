...
import { Search, MapPin, Building, ShieldCheck, SlidersHorizontal, ChevronLeft, ChevronRight, RotateCcw, BookOpen } from 'lucide-react';
...
const HERO_IMAGES = [
  ...
  },
  {
    ...
    title: 'Luxury Oceanfront Villas & Sea-Facing Residences',
...
export const HeroSection: React.FC<HeroSectionProps> = ({ onSearch, onOpenGuide }) => {
    ...
    <div className="relative min-h-[88vh] flex items-center justify-center overflow-hidden pt-6 pb-16 bg-slate-950">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 w-full pt-4">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-3 mb-6">
          <div className="inline-flex items-center justify-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1 sm:py-1.5 rounded-full bg-slate-900/80 border border-orange-500/40 text-[11px] sm:text-xs font-bold text-orange-400 shadow-xl backdrop-blur-md text-center max-w-full leading-tight">
            <ShieldCheck className="w-3.5 h-3.5 text-amber-400 shrink-0" />
            <span className="truncate sm:whitespace-normal">Pakistan's #1 Escrow Protected Property Platform</span>
          </div>
        ...
        {/* UNIFIED 55%-60% FROSTED GLASS SEARCH CARD */}
        <div className="bg-slate-900/60 backdrop-blur-2xl border border-white/20 shadow-[0_25px_60px_rgba(0,0,0,0.75)] rounded-3xl p-4 sm:p-7 space-y-4 ring-1 ring-white/10">
            ...
            <div className="grid grid-cols-2 sm:flex sm:flex-wrap items-center gap-2 w-full">
              {[
                ...
                <button
                  ...
                  className={`min-h-[38px] sm:min-h-[42px] px-3 sm:px-4 py-1.5 sm:py-2 inline-flex items-center justify-center rounded-xl text-[11px] sm:text-xs font-bold transition-all text-center leading-none whitespace-nowrap ${
                    purpose === tab.id
                      ? 'bg-gradient-to-r from-orange-500 via-amber-500 to-orange-600 text-white shadow-lg shadow-orange-500/30'
                      : 'text-slate-200 hover:text-white hover:bg-slate-800/80 bg-slate-950/60 border border-white/15 backdrop-blur-md'
                  }`}
                >
                  {tab.label}
                </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-2.5 items-center">
            ...
            {/* City Selector */}
            <div className="md:col-span-3 bg-slate-950/60 backdrop-blur-md border border-white/20 rounded-xl px-3 py-2.5 flex items-center space-x-2 focus-within:border-orange-500 transition-colors shadow-inner min-h-[42px]">
            ...
            {/* Keyword Input */}
            <div className="md:col-span-5 bg-slate-950/60 backdrop-blur-md border border-white/20 rounded-xl px-3 py-2.5 flex items-center space-x-2 focus-within:border-orange-500 transition-colors shadow-inner min-h-[42px]">
            ...
            <div className="md:col-span-4 flex items-center gap-2">
              <button
                ...
                className="flex-1 min-h-[42px] gradient-btn text-white py-2 sm:py-2.5 px-3 sm:px-4 rounded-xl font-bold text-xs sm:text-sm inline-flex items-center justify-center gap-2 shadow-lg shadow-orange-500/25 transition-all hover:brightness-110 leading-none whitespace-nowrap"
              >
              ...
              <button
                ...
                className={`min-h-[42px] py-2 sm:py-2.5 px-3 sm:px-3.5 rounded-xl border text-xs font-bold inline-flex items-center justify-center gap-1.5 transition-all shrink-0 leading-none whitespace-nowrap ${
                  showAdvance || activeAdvanceCount > 0
                    ? 'bg-orange-500/30 border-orange-500 text-orange-200'
                    : 'bg-slate-950/60 backdrop-blur-md border-white/20 text-slate-200 hover:bg-slate-800/80'
                }`}
              >
                <SlidersHorizontal className="w-4 h-4 text-orange-400 shrink-0" />
                <span>Filters</span>
                {activeAdvanceCount > 0 && (
                  <span className="w-4 h-4 rounded-full bg-orange-500 text-white text-[10px] inline-flex items-center justify-center font-bold leading-none">
                    {activeAdvanceCount}
                  </span>
                )}
              </button>
              ...
              {(activeAdvanceCount > 0 || keyword || city !== 'Islamabad') && (
                <button
                  type="button"
                  onClick={handleReset}
                  className="min-h-[42px] min-w-[42px] p-2.5 rounded-xl bg-slate-950/60 hover:bg-slate-800 text-slate-300 hover:text-white transition-colors shrink-0 border border-white/15 backdrop-blur-md inline-flex items-center justify-center"
                  title="Reset Search"
                >
            ...
            <div className="pt-3 border-t border-white/15 space-y-3 animate-in fade-in duration-200">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 text-left">
                ...
                <div>
                  ...
                  <div className="bg-slate-950/60 backdrop-blur-md border border-white/20 rounded-xl px-3 py-2 flex items-center space-x-2 focus-within:border-orange-500 shadow-inner">
                ...
                <div>
                  ...
                  <div className="bg-slate-950/60 backdrop-blur-md border border-white/20 rounded-xl px-3 py-2 focus-within:border-orange-500 shadow-inner">
                    <input
                      type="number"
              ...
              <div className="pt-2 flex justify-end">
                <button
                  onClick={handleExecuteSearch}
                  className="min-h-[38px] sm:min-h-[42px] gradient-btn text-white px-4 sm:px-6 py-1.5 sm:py-2 rounded-xl font-bold text-xs sm:text-sm shadow-lg shadow-orange-500/20 inline-flex items-center justify-center leading-none whitespace-nowrap"
                >
          ...
          <div className="pt-2 border-t border-white/15 flex flex-wrap items-center justify-between gap-2 text-xs">
            <div className="flex flex-wrap items-center gap-1.5">
              <span className="text-slate-300 font-medium text-[10px] sm:text-[11px] mr-1">Popular Searches:</span>
              {['DHA Phase 6 Lahore', 'E-11 Islamabad', 'Clifton Karachi', 'Bahria Town', 'Capital Smart City'].map(tag => (
                <button
                  ...
                  className="inline-flex items-center justify-center px-2 sm:px-2.5 py-1 sm:py-1.5 rounded-lg bg-slate-950/60 hover:bg-slate-800/80 text-slate-200 hover:text-white text-[10px] sm:text-[11px] font-medium border border-white/15 backdrop-blur-md transition-colors shadow-sm leading-none whitespace-nowrap"
                >
                  {tag}
                </button>
              ))}
