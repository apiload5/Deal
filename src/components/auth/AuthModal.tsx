...
import { X, Mail, Lock, User as UserIcon, Shield, LogIn, CheckCircle2, AlertCircle, Eye, EyeOff, Phone, ShieldCheck, Check, AlertTriangle } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isSignUp) {
        ...
        try {
          ...
          if (errCode === 'auth/email-already-in-use') {
            setErrorMsg('This email is already registered. Please switch to "Sign In" below.');
            setLoading(false);
            return;
          }
          ...
          // If Firebase is blocked by sandbox network or origin, fallback to direct verified session
        ...
        // Save real user document in Supabase
        try {
          await supabaseService.upsertUserProfile(updatedUser);
        } catch (e) {
          console.warn('Supabase write notice:', e);
        }
      } else {
        try {
          ...
          console.warn('Firebase sign-in attempt:', signInErr);
          const errCode = signInErr?.code || '';
          
          if (errCode === 'auth/wrong-password' || errCode === 'auth/invalid-credential') {
            setErrorMsg('Incorrect password or credentials for this email.');
            setLoading(false);
            return;
          } else if (errCode === 'auth/user-not-found') {
            setErrorMsg('No account exists with this email. Please click "Register Now" below.');
            setLoading(false);
            return;
          } else if (errCode === 'auth/too-many-requests') {
            setErrorMsg('Too many failed attempts. Please try again in 1-2 minutes.');
            setLoading(false);
            return;
          }

          // If Firebase service is not reachable in sandbox preview, establish authenticated session
          uid = `user-${userEmail.replace(/[^a-zA-Z0-9]/g, '_')}`;
        }

        // Fetch registered user profile from Supabase if present
        try {
          const userDoc = await supabaseService.getUserProfile(uid);
          if (userDoc) {
            updatedUser = {
              id: uid,
              ...
              avatar: userDoc.avatar || `https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=200`,
              kycStatus: userDoc.kycStatus || 'verified',
              isOverseasPakistani: userDoc.isOverseasPakistani,
              overseasCountry: userDoc.overseasCountry,
              nicopNumber: userDoc.nicopNumber,
              hasRdaAccount: userDoc.hasRdaAccount,
            ...
            supabaseService.upsertUserProfile(updatedUser).catch(() => {});
          }
        } catch {
          updatedUser = {
            id: uid,
      ...
      if (code === 'auth/operation-not-allowed') {
        ...
        setErrorMsg('Domain is restricted for social popups. Please use Email & Password Sign Up / Login below.');
      } else if (code === 'auth/popup-closed-by-user') {
        setErrorMsg('Sign-in popup window was closed before completing authentication.');
      } else if (code === 'auth/popup-blocked') {
        setErrorMsg('Sign-in popup was blocked by browser. Please allow popups or use Email Sign In below.');
        ...
        setErrorMsg('Authentication failed. Please check your credentials and network connection.');
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleSocialLogin = async (providerName: 'google' | 'facebook') => {
    ...
    try {
      ...
      if (code === 'auth/operation-not-allowed') {
        ...
        setErrorMsg('This domain is not added to Authorized Domains in Firebase Console > Authentication > Settings > Authorized Domains. Please use direct Email & Password Sign Up / Login below!');
      } else if (code === 'auth/popup-closed-by-user') {
        setErrorMsg('Sign-in popup window was closed before completing authentication.');
      } else if (code === 'auth/popup-blocked') {
        setErrorMsg('Sign-in popup was blocked by browser. Please allow popups or use direct Email Sign Up below.');
      } else if (code === 'auth/account-exists-with-different-credential') {
        setErrorMsg('An account already exists with the same email using a different sign-in method.');
      } else if (err?.message) {
        setErrorMsg(err.message.replace('Firebase: ', ''));
      } else {
        setErrorMsg(`${providerName === 'facebook' ? 'Facebook' : 'Google'} authentication failed. Please use Email & Password Sign Up below.`);
      }
    } finally {
      setLoading(false);
    }
