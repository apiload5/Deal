...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ onBackToWebsite, onOpenChat }) => {
  ...
  const [workerCreateError, setWorkerCreateError] = useState<string | null>(null);
  const [confirmDeleteStaffId, setConfirmDeleteStaffId] = useState<string | null>(null);
  const [confirmPurgeAllStaff, setConfirmPurgeAllStaff] = useState(false);
  ...
  const [raastId, setRaastId] = useState(store.bankDetails?.raastId || '03182055632');
  const [raastIban, setRaastIban] = useState(store.bankDetails?.raastIban || store.bankDetails?.iban || 'PK92MEZN0001020304050607');
  ...
  const handleCreateStaff = async (e: React.FormEvent) => {
    e.preventDefault();
    setWorkerCreateError(null);
    if (!newStaffName.trim() || !newStaffUsername.trim() || !newStaffPassword.trim() || !newStaffPin.trim()) {
      setWorkerCreateError('Please fill in all worker credentials (Name, Username, Password, and PIN).');
      return;
    }

    if (!pinMakerPasswordInput.trim()) {
      setWorkerCreateError('🔒 Security Authorization Mandate: Pin Maker Password is required to create a worker account and generate their security PIN!');
      return;
    }
    ...
    if (!isPinMakerOk) {
      ...
      setWorkerCreateError('❌ AUTHORIZATION DENIED: Invalid Pin Maker Password! Verification failed against salt/hash.');
      return;
    }

    const passHash = await hashWithBcrypt(newStaffPassword.trim(), 12);
    const pinHash = await hashWithBcrypt(newStaffPin.trim(), 12);
    ...
    showToast(`Worker @${newStaffUsername.trim().toLowerCase()} created successfully!`);
  ...
  const handleToggleStaffPermission = (staffId: string, key: keyof StaffPermissions) => {
    ...
    if (staff) {
      ...
      showToast(`Permissions updated for ${staff.name}`);
    }
  };

  const handleDeleteStaff = (staffId: string) => {
    const target = store.staffUsers.find(s => s.id === staffId);
    const targetName = target ? target.name : 'Worker';
    store.deleteStaffUser(staffId);
    setConfirmDeleteStaffId(null);
    setRefreshToggle(prev => prev + 1);
    showToast(`Worker account (${targetName}) revoked and deleted permanently.`);
  };

  const handlePurgeAllStaff = () => {
    store.clearAllStaffUsers();
    setConfirmPurgeAllStaff(false);
    setRefreshToggle(prev => prev + 1);
    showToast('All worker accounts purged and removed from system database.');
  };
  ...
  // Dynamic Real Pipeline Deals
  const rawPipelineDeals = store.bookings.length > 0
    ? store.bookings.slice(0, 5).map((b) => ({
        id: b.id,
        title: b.propertyTitle,
        buyer: b.buyerName,
        seller: b.sellerName,
        amount: `PKR ${b.amountPaid.toLocaleString('en-PK')}`,
        step: b.paymentStatus === 'escrow_held' ? 3 : 4,
        stepName: b.paymentStatus === 'escrow_held' ? 'Token Escrow Locked' : 'Settled & Completed',
        status: 'on_track',
        color: 'green'
      }))
    : store.properties.slice(0, 3).map((p) => ({
        id: p.id,
        title: p.title,
        buyer: 'Verified Inquirer',
        seller: p.ownerName || 'Verified Agency',
        amount: p.priceFormatted,
        step: 2,
        stepName: 'Active Listing Pipeline',
        status: 'on_track',
        color: 'green'
      }));
  ...
  // Dynamic Real Activity Feed from System Security & Audit Logs
  const rawActivityFeed = store.securityAuditLogs.length > 0
    ? store.securityAuditLogs.slice(0, 8).map(log => ({
        id: log.id,
        text: `${log.user}: ${log.event}`,
        time: new Date(log.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        type: log.status === 'blocked' ? 'dispute' : log.status === 'alert' ? 'proof' : 'escrow'
      }))
    : [
        { id: 'act-1', text: 'System Security Kernel Active & Monitoring Network', time: 'Live', type: 'bounty' },
        { id: 'act-2', text: 'Bcrypt Hash & Salt Verification Engine Initialized', time: 'Live', type: 'proof' }
      ];
            ...
            <div className={`lg:col-span-5 p-6 rounded-3xl border space-y-5 ${cardClasses}`}>
              ...
              <div className="space-y-2 pt-2 border-t border-slate-800">
                ...
                <div className="space-y-2 text-xs">
                  {store.financialAuditLogs.length > 0 ? (
                    store.financialAuditLogs.slice(0, 4).map(tx => (
                      <div key={tx.id} className="flex justify-between items-center p-2 rounded-xl bg-slate-950/80 border border-slate-800/80 text-[11px]">
                        <div>
                          <p className="font-bold text-white capitalize">{tx.txType.replace('_', ' ')} • {tx.referenceNo}</p>
                          <p className="text-[9px] text-slate-500">{new Date(tx.timestamp).toLocaleDateString()} via {tx.gateway}</p>
                        </div>
                        <span className={`font-bold ${tx.txType === 'wallet_deposit' || tx.txType === 'escrow_lock' ? 'text-emerald-400' : 'text-orange-400'}`}>
                          PKR {tx.amountPKR.toLocaleString('en-PK')}
                        </span>
                      </div>
                    ))
                  ) : store.bookings.length > 0 ? (
                    store.bookings.slice(0, 3).map(b => (
                      <div key={b.id} className="flex justify-between items-center p-2 rounded-xl bg-slate-950/80 border border-slate-800/80 text-[11px]">
                        <div>
                          <p className="font-bold text-white">{b.propertyTitle}</p>
                          <p className="text-[9px] text-slate-500">Buyer: {b.buyerName}</p>
                        </div>
                        <span className="font-bold text-emerald-400">
                          PKR {b.amountPaid.toLocaleString('en-PK')}
                        </span>
                      </div>
                    ))
                  ) : (
                    <div className="p-3 text-center rounded-xl bg-slate-950/50 border border-slate-800/60 text-slate-500 text-[11px]">
                      No active escrow ledger transactions recorded.
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-3">
                {store.agents.length === 0 ? (
                  <div className="p-6 text-center rounded-2xl bg-slate-950/50 border border-slate-800 text-slate-500 text-xs">
                    ...
                    <p className="font-bold text-slate-400">No Verified Agents Registered</p>
                    <p className="text-[10px] text-slate-500 mt-0.5">Agents onboarded to DealFast will appear here.</p>
                  </div>
                ) : (
      ...
      {activeTab === 'staff' && isSuperAdmin && (
        <div className="space-y-6">
          ...
          <form onSubmit={handleCreateStaff} className="glass-card rounded-2xl p-6 border border-purple-500/30 space-y-4 text-xs">
            ...
            {workerCreateError && (
              <div className="p-3 rounded-xl bg-red-500/10 border border-red-500/30 text-red-400 font-bold text-xs flex items-center">
                <AlertTriangle className="w-4 h-4 mr-2 flex-shrink-0" />
                <span>{workerCreateError}</span>
              </div>
            )}

            {staffCreatedSuccess && (
              <p className="text-emerald-400 font-bold text-xs flex items-center">
                <CheckCircle2 className="w-4 h-4 mr-1" /> Worker account successfully created & worker PIN securely generated!
              </p>
            )}
          ...
          <div className="space-y-3">
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              {staffList.length > 0 && (
                confirmPurgeAllStaff ? (
                  <div className="flex items-center space-x-2 bg-red-950/80 border border-red-500/50 px-3 py-1 rounded-xl">
                    <span className="text-[11px] text-red-300 font-bold">Purge ALL workers?</span>
                    <button
                      onClick={handlePurgeAllStaff}
                      className="px-2.5 py-0.5 bg-red-600 hover:bg-red-500 text-white text-[10px] font-bold rounded-lg"
                    >
                      Yes, Wipe All
                    </button>
                    <button
                      onClick={() => setConfirmPurgeAllStaff(false)}
                      className="px-2 py-0.5 bg-slate-800 hover:bg-slate-700 text-slate-300 text-[10px] rounded-lg"
                    >
                      Cancel
                    </button>
                  </div>
                  ...
                  <button
                    onClick={() => setConfirmPurgeAllStaff(true)}
                    className="px-3 py-1 rounded-xl bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/30 text-[11px] font-bold flex items-center space-x-1 transition"
                    title="Purge all worker accounts from database"
                  >
                    <Trash2 className="w-3.5 h-3.5" />
                    <span>Purge / Clean All Workers</span>
                  </button>
                )
              )}
            </div>
                  ...
                  {staffList.length === 0 ? (
                    ...
                    <tr key={s.id} className="hover:bg-slate-900/50">
                      ...
                      <td className="p-3 font-mono text-slate-400">
                        <span className="px-2 py-1 rounded bg-slate-950 border border-slate-800 text-amber-400/90 font-bold text-[11px] tracking-widest flex items-center w-max">
                          ••••••
                        </span>
                      </td>
                      <td className="p-3">
                        ...
                        <div className="flex items-center space-x-2">
                          <button
                            onClick={() => {
                              ...
                              else showToast(`Direct Encrypted Chat launched with ${s.name}`);
                            }}
                          ...
                          {confirmDeleteStaffId === s.id ? (
                            <div className="flex items-center space-x-1 bg-red-950/90 border border-red-500/60 p-1 rounded-xl">
                              <span className="text-[10px] text-red-300 font-bold px-1">Revoke?</span>
                              <button
                                onClick={() => handleDeleteStaff(s.id)}
                                className="px-2 py-0.5 bg-red-600 hover:bg-red-500 text-white rounded-lg text-[10px] font-bold"
                              >
                                Yes, Delete
                              </button>
                              <button
                                onClick={() => setConfirmDeleteStaffId(null)}
                                className="px-1.5 py-0.5 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg text-[10px]"
                              >
                                Cancel
                              </button>
                            </div>
                            ...
                            <button
                              ...
                              className="p-1.5 rounded-lg bg-red-600/20 hover:bg-red-600/40 text-red-400 border border-red-500/30 flex items-center space-x-1 text-[10px] font-bold transition-colors"
        ...
        <div className="space-y-4 text-xs">
          ...
          <div className="glass-card rounded-2xl border border-slate-800 overflow-x-auto">
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-slate-800/60">
                {[
                  { name: store.currentUser.name || 'Super Admin', email: store.currentUser.email || 'ahmadraza2580421@gmail.com', role: store.currentUser.role, kyc: store.currentUser.kycStatus || 'verified' },
                  ...store.staffUsers.map(s => ({ name: s.name, email: `@${s.username}`, role: `worker (${s.roleTitle})`, kyc: 'verified' })),
                  ...store.agencies.map(a => ({ name: a.name, email: a.email, role: 'agency', kyc: 'verified' })),
                  ...store.agents.map(ag => ({ name: ag.name, email: ag.email || ag.phone, role: 'agent', kyc: 'verified' })),
                  ...store.builders.map(b => ({ name: b.name, email: b.email || b.phone, role: 'builder', kyc: 'verified' }))
                ].map((u, idx) => (
                  <tr key={idx} className="hover:bg-slate-900/50">
                    ...
                    <td className="p-3">
                      <span className="text-emerald-400 font-bold flex items-center">
                        <CheckCircle2 className="w-3.5 h-3.5 mr-1" /> {u.kyc || 'Verified'}
                      </span>
                    </td>
                    <td className="p-3">
                      <span className="text-slate-400 text-[11px] font-medium">Active Account</span>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="space-y-4 text-xs">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-bold text-white text-sm flex items-center gap-2">
                <span>Escrow Safe Release & Raast Settlement Manager</span>
              ...
              <p className="text-slate-400 text-[11px]">Audit and release zero-fee State Bank Raast deposits and buyer escrow holdings</p>
            </div>
            <span className="text-slate-400 font-mono">{allBookings.length} total escrow transactions</span>
          </div>
          ...
          {allBookings.length === 0 ? (
            <div className="p-8 text-center bg-slate-900/50 rounded-2xl border border-slate-800 text-slate-400 space-y-2">
              ...
              <p className="font-bold text-white text-xs">No active escrow transactions</p>
              <p className="text-[11px]">Escrow token bookings will be listed here with SBP RRN verification details.</p>
            </div>
          ) : (
            allBookings.map(b => (
              <div key={b.id} className="glass-card rounded-2xl p-5 border border-slate-800 space-y-3">
                    ...
                    <h4 className="font-bold text-white text-sm flex items-center gap-2">
                      ...
                      {b.paymentMethod === 'raast' && (
                        <span className="bg-emerald-500/20 text-emerald-300 text-[9px] font-black px-2 py-0.5 rounded-full border border-emerald-500/40">
                          ⚡ State Bank Raast (0% Fee)
                        </span>
                      )}
                    </h4>
                    <p className="text-[11px] text-slate-400 mt-0.5">
                      Buyer: <strong className="text-slate-200">{b.buyerName}</strong> ({b.buyerPhone}) ➔ Seller: <strong className="text-slate-200">{b.sellerName}</strong>
                    </p>
                  </div>
                  <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase ${
                    b.paymentStatus === 'released' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' :
                    b.paymentStatus === 'refunded' ? 'bg-rose-500/20 text-rose-400 border border-rose-500/30' :
                    'bg-amber-500/20 text-amber-400 border border-amber-500/30'
                  }`}>
                    Status: {b.paymentStatus.replace('_', ' ')}
                  </span>
                </div>

                {/* Raast Verification Strip */}
                {b.raastRrn && (
                  <div className="p-3 rounded-xl bg-slate-950 border border-emerald-500/30 flex flex-wrap items-center justify-between gap-2 text-[11px]">
                    <div className="flex items-center space-x-2">
                      <span className="text-emerald-400 font-bold">SBP Raast RRN:</span>
                      <code className="bg-slate-900 text-amber-300 px-2 py-0.5 rounded font-mono font-bold tracking-wider">{b.raastRrn}</code>
                      {b.raastEscrowRef && (
                        <span className="text-slate-400 text-[10px]">Ref: {b.raastEscrowRef}</span>
                      )}
                    </div>
                    <span className="text-emerald-400 text-[10px] font-bold">
                      Zero 3rd-Party Gateway Fee • 100% Escrow Intact
                    </span>
                  </div>
                )}
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 pt-1">
                  <p className="font-black text-white text-base">
                    Amount: <span className="text-emerald-400">PKR {b.amountPaid.toLocaleString('en-PK')}</span>
                  </p>
        ...
        <div className="space-y-6 max-w-4xl">
          <form onSubmit={e => {
            ...
            <div className="pt-4 border-t border-slate-800 space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-300 font-bold mb-1">State Bank Official Raast ID (Mobile)</label>
                  <input
                    type="text"
                    value={raastId}
                    onChange={e => setRaastId(e.target.value)}
                    className="bg-slate-900 border border-slate-800 rounded-xl px-3 py-2 text-emerald-400 font-bold w-full outline-none focus:border-emerald-500"
                  />
                </div>
                <div>
                  <label className="block text-slate-300 font-bold mb-1">State Bank Raast IBAN</label>
                  <input
                    type="text"
                    value={raastIban}
                    onChange={e => setRaastIban(e.target.value)}
                    className="bg-slate-900 border border-slate-800 rounded-xl px-3 py-2 text-white font-medium w-full outline-none focus:border-emerald-500"
                  />
                </div>
              </div>
            ...
            <button
              ...
              onClick={async () => {
                ...
                const selectedSender = testEmailDepartment === 'payments' ? paymentsEmail
                  ...
                  : infoEmail;
                const apiKeyToUse = emailProvider === 'brevo' ? brevoApiKey
                ...
                try {
                  ...
                  if (data.success) {
                    setTestEmailResult(data.message || `Test email successfully dispatched to ${testRecipientEmail}!`);
                  } else {
                    setTestEmailResult(`⚠️ Email Dispatch Error: ${data.error || 'Server dispatch failed'}`);
                  }
                } catch (err: any) {
                  setTestEmailResult(`⚠️ Network Error: ${err?.message || 'Failed to reach server email endpoint'}`);
                } finally {
