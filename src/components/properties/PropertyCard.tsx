      ...
      <div className={`relative bg-slate-900 overflow-hidden shrink-0 ${
        ...
        <div className="absolute top-3 left-3 flex flex-wrap items-center gap-1.5 z-20">
          {(property.isFeatured || property.isPremium) && (
            <span className="px-2.5 py-0.5 rounded-md text-[10px] font-black uppercase tracking-wider bg-gradient-to-r from-amber-400 via-yellow-300 to-amber-500 text-slate-950 flex items-center shadow-lg border border-yellow-200 ring-2 ring-amber-500/30">
              <Sparkles className="w-3 h-3 mr-1 text-slate-950" /> ⭐ FEATURED TOP AD
            </span>
          )}
          {relevanceScore > 0 && (
            <span className="px-2 py-0.5 rounded-md text-[10px] font-black uppercase bg-emerald-400 text-slate-950 flex items-center shadow-md">
              <Target className="w-3 h-3 mr-1 text-slate-950" /> 🎯 Matched For You
            </span>
          )}
      ...
      <div className={`p-4 sm:p-5 flex-1 flex flex-col justify-between space-y-3 ${isList ? 'md:py-4' : ''}`}>
        ...
        <div className="pt-1 flex items-center justify-between gap-2 flex-wrap">
          <div className="flex items-center space-x-2 min-w-0">
            <div className="w-8 h-8 rounded-full bg-slate-800 text-orange-400 font-bold flex items-center justify-center text-xs shrink-0 border border-slate-700">
          ...
          <div className="flex items-center space-x-1.5">
            <button
              ...
              className="p-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 transition-colors flex items-center space-x-1"
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-xl gradient-btn text-white text-[11px] font-bold transition-all shadow-md shadow-orange-500/20"
            >
              Book Token (10%)
            </button>
          </div>
