...
import { X, Plus, Star, Building2, Building, MapPin, CheckCircle2, ShieldCheck, Phone, Video, PhoneCall, MessageSquare, Clock, Calendar, Send, Award, UserCheck } from 'lucide-react';
...
export const PropertyFormModal: React.FC<PropertyFormModalProps> = ({
  ...
  const [listerRole, setListerRole] = useState<'user' | 'agent' | 'builder' | 'agency'>('user');
  const [listerAgencyName, setListerAgencyName] = useState('');
  ...
  useEffect(() => {
    if (propertyToEdit) {
      ...
      setListerRole((propertyToEdit.userRole as any) || 'user');
      setListerAgencyName(propertyToEdit.agencyName || '');
      ...
      const defaultRole = u.role === 'agent' ? 'agent' : u.role === 'builder' ? 'builder' : u.role === 'agency' ? 'agency' : 'user';
      setListerRole(defaultRole);
      setListerAgencyName(u.agencyName || u.companyName || '');
    }
  ...
  const handleFinalSubmit = (e: React.FormEvent) => {
    ...
    if (propertyToEdit) {
      resultProperty = store.updateProperty(propertyToEdit.id, {
        ...
        userRole: listerRole,
        agencyName: listerRole === 'user' ? undefined : (listerAgencyName.trim() || user.agencyName || user.companyName || undefined),
        lat: lat || 24.8607,
        lng: lng || 67.0011,
        ...
        <form onSubmit={handleFinalSubmit} className="space-y-4 text-xs">
          ...
          {step === 1 && (
            <div className="space-y-4">
              {/* Lister Identity & Authentic Representation Selector */}
              <div className="p-3.5 rounded-2xl bg-slate-950/80 border border-slate-800 space-y-2.5 shadow-inner">
                <div className="flex items-center justify-between">
                  <label className="font-bold text-white text-xs flex items-center">
                    <UserCheck className="w-4 h-4 text-orange-400 mr-1.5" />
                    Lister Representation Status *
                  </label>
                  <span className="text-[10px] text-slate-400">Accurate & Verified Badge</span>
                </div>
                
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                  <button
                    type="button"
                    onClick={() => setListerRole('user')}
                  ...
                  >
                    <div className="font-bold text-xs flex items-center gap-1">
                      <ShieldCheck className="w-3.5 h-3.5 text-emerald-400 shrink-0" /> Direct Owner
                    </div>
                    <div className="text-[9px] text-slate-400 font-normal mt-0.5">ذاتی مالک (0% Comm)</div>
                  </button>
                  ...
                  <button
                    ...
                    className={`p-2.5 rounded-xl border text-left transition-all ${
                      listerRole === 'agent'
                        ? 'bg-blue-500/20 border-blue-500 text-blue-300 font-bold shadow-md shadow-blue-500/10'
                        : 'bg-slate-900 border-slate-800 text-slate-400 hover:text-slate-200'
                    }`}
                  >
                    <div className="font-bold text-xs flex items-center gap-1">
                      <Award className="w-3.5 h-3.5 text-blue-400 shrink-0" /> Certified Agent
                    </div>
                    <div className="text-[9px] text-slate-400 font-normal mt-0.5">رجسٹرڈ ایجنٹ / ڈیلر</div>
                  </button>

                  <button
                    type="button"
                    onClick={() => setListerRole('builder')}
                    className={`p-2.5 rounded-xl border text-left transition-all ${
                      listerRole === 'builder'
                  ...
                  >
                    <div className="font-bold text-xs flex items-center gap-1">
                      <Building2 className="w-3.5 h-3.5 text-purple-400 shrink-0" /> Builder / Dev
                    </div>
                    <div className="text-[9px] text-slate-400 font-normal mt-0.5">بلڈر و ڈویلپر</div>
                  </button>

                  <button
                    type="button"
                    onClick={() => setListerRole('agency')}
                    className={`p-2.5 rounded-xl border text-left transition-all ${
                      listerRole === 'agency'
                  ...
                  >
                    <div className="font-bold text-xs flex items-center gap-1">
                      <Building className="w-3.5 h-3.5 text-amber-400 shrink-0" /> Agency
                    </div>
                    <div className="text-[9px] text-slate-400 font-normal mt-0.5">ایجنسی بروکرز</div>
                  </button>
                </div>

                {(listerRole === 'agent' || listerRole === 'agency' || listerRole === 'builder') && (
                  <div className="pt-2 animate-in fade-in">
                    <label className="block text-[10px] font-bold text-slate-300 uppercase tracking-wider mb-1">
                      {listerRole === 'builder' ? 'Builder / Developer Project Name *' : 'Agency / Brokerage Name *'}
                    </label>
                    <input
                      type="text"
                      placeholder={listerRole === 'builder' ? 'e.g. Al-Miraj Builders & Developers' : 'e.g. DHA Premier Real Estate'}
                      value={listerAgencyName}
                      onChange={e => setListerAgencyName(e.target.value)}
                      className="w-full bg-slate-900 border border-slate-800 rounded-xl p-2.5 text-white outline-none focus:border-orange-500 text-xs"
                    />
                  </div>
                )}
              </div>
            ...
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-amber-400">DealFast Top Spot & Targeted Buyers</p>
                </div>
              </div>
              <button
                type="button"
