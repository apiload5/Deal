...
export const WalletModal: React.FC<WalletModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleTopup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Execute topup on store & Firestore with verified gateway transaction reference ID
      store.topupWallet(amount, paymentMethod, data.referenceId);
      setSuccessMessage(`PKR ${amount.toLocaleString()} deposited successfully into your Escrow Wallet via ${paymentMethod.toUpperCase()}! (Ref: ${data.referenceId})`);
      setTopupAmount('');
  ...
  const handleWithdraw = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        throw new Error(data.error || 'Withdrawal processing failed at payment gateway.');
      }

      const withdrawOk = store.withdrawWallet(amount, paymentMethod, accountDetails);
      if (withdrawOk) {
        setSuccessMessage(`PKR ${amount.toLocaleString()} withdrawal request processed and recorded! (Ref: ${data.referenceId || 'WD-OK'})`);
        setWithdrawAmount('');
        setAccountDetails('');
        setActiveTab('balance');
      } else {
