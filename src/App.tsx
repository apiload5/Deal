...
import { BoostListingModal } from './components/properties/BoostListingModal';
import { sortPropertiesWithOLXAndFBAlgorithm, getUserAffinities, trackPropertyView, trackSearchPreference } from './lib/recommendation';
...
export default function App() {
  ...
  const sortPropertiesList = (list: Property[], sortBy: string = 'newest') => {
    const profile = getUserAffinities();
    return sortPropertiesWithOLXAndFBAlgorithm(list, sortBy, profile);
  };

  const initialSortedProps = sortPropertiesList(store.properties || []);
  ...
  useEffect(() => {
    const unsubscribe = store.subscribe(() => {
      ...
      handleFilterSearch(searchFilters);
    });
    return () => unsubscribe();
  }, [searchFilters]);
  ...
  const handleFilterSearch = (filters: SearchFilter) => {
    ...
    if (filters) {
      trackSearchPreference(filters);
    }
    let result = (store.properties || []).filter(p =>
      p && p.status !== 'recycle_bin' && (p.status === 'approved' || p.userId === store.currentUser.id || store.currentUser.role === 'admin')
    );
    if (!filters) {
      setFilteredProperties(sortPropertiesList(result, 'newest'));
      return;
    }
    ...
    setFilteredProperties(sortPropertiesList(result, filters.sortBy));
  };
  ...
  const handleStartCall = (agentName: string, agentAvatar?: string, isVideo: boolean = true, agentId?: string) => {
    handleRequireAuth(() => {
  ...
  return (
    ...
    <div className="min-h-screen bg-[#0a0e1a] text-slate-100 flex flex-col font-sans selection:bg-orange-500 selection:text-white">
      ...
      {incomingCall && (
        <div className="fixed top-6 right-6 z-[10000] w-96 bg-slate-950 border-2 border-orange-500 shadow-2xl rounded-2xl p-4 animate-bounce">
          <div className="flex items-center space-x-3 mb-3">
            <div className="relative">
              <img
                src={incomingCall.callerAvatar || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=200'}
                alt={incomingCall.callerName}
                className="w-12 h-12 rounded-full border-2 border-orange-500 object-cover"
              />
              <span className="absolute -top-1 -right-1 w-4 h-4 bg-emerald-500 rounded-full ring-2 ring-slate-950 animate-ping" />
            </div>
            <div className="flex-1 truncate">
              <p className="text-xs font-bold text-orange-400 uppercase tracking-wider">Incoming WebRTC Call</p>
              <h4 className="text-sm font-bold text-white truncate">{incomingCall.callerName || 'Property Buyer'}</h4>
              <p className="text-[10px] text-slate-400">Requesting {incomingCall.isVideo ? 'Video' : 'Voice'} Call...</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-800">
            <button
              onClick={() => {
                ...
                setCallAgentName(call.callerName || 'Caller');
                setCallAgentAvatar(call.callerAvatar);
                setCallAgentId(call.callerId);
                setIsCallVideo(call.isVideo !== false);
                setIsCallActive(true);
              }}
              className="py-2.5 px-3 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-bold flex items-center justify-center space-x-1.5 shadow-lg shadow-emerald-600/30 transition-all"
            >
            ...
            <button
              ...
              className="py-2.5 px-3 bg-red-600/20 hover:bg-red-600/40 border border-red-500/30 text-red-400 rounded-xl text-xs font-bold flex items-center justify-center space-x-1.5 transition-all"
            >
              <span>Decline</span>
            </button>
          </div>
