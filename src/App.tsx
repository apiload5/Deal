...
export default function App() {
  ...
  const [isIncomingAnswer, setIsIncomingAnswer] = useState<boolean>(false);
  ...
  const handleStartCall = (agentName: string, agentAvatar?: string, isVideo: boolean = true, agentId?: string) => {
    handleRequireAuth(() => {
      ...
      setIsIncomingAnswer(false);
      setIsCallActive(true);
    });
  };

  const handleAcceptIncomingCall = (call: any) => {
    setCallAgentName(call.callerName || 'Property Buyer');
    ...
    setIsIncomingAnswer(true);
    setIsCallActive(true);
  };

  const handleSelectProperty = (p: Property | null) => {
    if (p) {
      store.recordUniquePropertyView(p.id);
      const updated = store.properties.find(item => item.id === p.id) || p;
      setSelectedProperty(updated);
      trackPropertyView(updated);
    } else {
  ...
  return (
    ...
    <div className="min-h-screen bg-[#0a0e1a] text-slate-100 flex flex-col font-sans selection:bg-orange-500 selection:text-white">
      ...
      <Navbar
        ...
        unreadNotifsCount={(store.notifications || []).filter(n => !n.isRead && (store.currentUser.id === 'guest' || store.currentUser.id === 'user-guest' ? (n.userId === 'all' || n.userId === 'guest') : (!n.userId || n.userId === 'all' || n.userId === store.currentUser.id))).length}
      ...
      <WebRTCCallModal
        ...
        isIncomingAnswer={isIncomingAnswer}
        onEndCall={() => {
      ...
      {/* 📞 INCOMING CALL RINGING MODAL WITH REAL FIRESTORE SIGNALING */}
      <IncomingCallModal
