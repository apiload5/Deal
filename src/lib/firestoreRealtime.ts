...
import { User, UserRole, ChatRoom, ChatMessage, AppNotification, CallLog } from '../types';
...
class FirestoreRealtimeService {
      ...
      const unsubscribe = onSnapshot(
        ...
        (snapshot) => {
          ...
          snapshot.forEach((docSnap) => {
            ...
            const isLastSenderMe = data.lastSenderId === userId;
            const isLastReadByMe = data.lastReadUserId === userId;
            let calculatedUnread = 0;
            if (isLastSenderMe || isLastReadByMe || data.isRead === true) {
              calculatedUnread = 0;
            } else if (rawUnread !== undefined) {
              calculatedUnread = Number(rawUnread) || 0;
            } else if (data.lastSenderId && data.lastSenderId !== userId) {
              calculatedUnread = 1;
            }
  ...
  public async markRoomMessagesAsRead(roomId: string, currentUserId: string): Promise<void> {
    ...
    try {
      ...
      // Also mark the room document as read for current user
      const roomRef = doc(db, 'chatRooms', roomId);
      updates.push(
        setDoc(roomRef, {
          unreadCount: 0,
          lastReadUserId: currentUserId,
          isRead: true,
          updatedAt: serverTimestamp()
        }, { merge: true })
      );
      ...
      console.warn('Firestore markRoomMessagesAsRead error:', e);
    }
  }
    ...
    try {
      ...
      snap.forEach((docSnap) => {
        if (!docSnap.data().isRead) {
          updates.push(updateDoc(docSnap.ref, { isRead: true, updatedAt: serverTimestamp() }));
        }
      });
      if (updates.length > 0) {
        await Promise.all(updates);
  ...
  public async createWebRTCCall(callData: {
    ...
    const callId = `call_${callData.callerId.replace(/[^a-zA-Z0-9_-]/g, '')}_${callData.receiverId.replace(/[^a-zA-Z0-9_-]/g, '')}_${Date.now()}`;
    try {
      ...
      await setDoc(callRef, {
        ...
        status: 'offering',
        offer: callData.offer ? { type: callData.offer.type, sdp: callData.offer.sdp } : null,
        createdAt: serverTimestamp()
      }, { merge: true });
    } catch (err) {
      console.warn('Firestore call record created with local/socket fallback:', err);
    }

    return { callId };
  }

  public async getCall(callId: string): Promise<WebRTCCallSession | null> {
  ...
  public subscribeToIncomingCalls(
    ...
    try {
      ...
      return onSnapshot(
        ...
        (error) => {
          console.warn('Firestore incoming call listener notice:', error.message);
        }
      );
    } catch (e) {
      return () => {};
  ...
  public async setCallOffer(callId: string, offerSdp: RTCSessionDescriptionInit): Promise<void> {
    ...
    try {
      ...
      console.warn('Firestore setCallOffer notice:', e);
    }
  }
  ...
  public async setCallAnswer(callId: string, answerSdp: RTCSessionDescriptionInit): Promise<void> {
    ...
    try {
      ...
      console.warn('Firestore setCallAnswer notice:', e);
    }
  }
  ...
  public listenToCall(callId: string, onUpdate: (call: WebRTCCallSession) => void): () => void {
    ...
    try {
      ...
      return onSnapshot(
        ...
        (error) => {
          console.warn('Firestore call listen notice:', error.message);
        }
      );
    } catch (e) {
      return () => {};
  ...
  public subscribeToIceCandidates(
    ...
    try {
      ...
      return onSnapshot(
        ...
        (error) => {
          console.warn('Firestore ICE candidate listen notice:', error.message);
        }
      );
    } catch (e) {
      return () => {};
  ...
  // 6. CALL LOGS & HISTORY
  // ==========================================
  public async saveCallLog(log: CallLog): Promise<void> {
    if (!log || !log.id) return;
    try {
      const logRef = doc(db, 'callLogs', log.id);
      await setDoc(logRef, {
        ...log,
        participantIds: [log.callerId, log.receiverId].filter(Boolean),
        createdAt: serverTimestamp()
      }, { merge: true });
    } catch (err) {
      console.warn('Firestore saveCallLog notice:', err);
    }
  }

  public subscribeToUserCallLogs(
    userId: string,
    onUpdate: (logs: CallLog[]) => void
  ): () => void {
    if (!userId || userId === 'guest') return () => {};
    try {
      const q = query(
      ...
      return onSnapshot(
        ...
        (snapshot) => {
          ...
          snapshot.forEach((docSnap) => {
            ...
            logs.push({
              ...
              type: data.callerId === userId ? 'outgoing' : (data.status === 'missed' || data.status === 'rejected' ? 'missed' : 'incoming'),
              status: data.status || 'ended',
              durationSeconds: data.durationSeconds || 0,
              timestamp: data.timestamp || 'Just now',
              createdAt: data.createdAt ? new Date(data.createdAt.toMillis?.() || Date.now()).toISOString() : new Date().toISOString()
            });
          });
        ...
        (error) => {
          console.warn('Firestore call logs listen notice:', error.message);
        }
      );
    } catch (e) {
      return () => {};
  ...
  public async deleteCallLog(logId: string): Promise<void> {
    if (!logId) return;
    try {
