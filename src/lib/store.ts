...
import { authService, propertyService, bookingService, chatService, walletService } from '../services';
...
export class AppStore {
  ...
  private firestoreChatUnsub: (() => void) | null = null;
  private firestoreNotifUnsub: (() => void) | null = null;
  private firestoreCallLogsUnsub: (() => void) | null = null;

  // 🔥 FIX BUG #2: Transaction Locking & Daily Withdrawal Limits Tracking
  private pendingTransactions: Map<string, { amount: number; timestamp: number }> = new Map();
  private dailyWithdrawalLimits: Map<string, { date: string; total: number }> = new Map();
  ...
  private constructor() {
    ...
    const logs = getStored<CallLog[]>(STORAGE_KEYS.CALL_LOGS, []);
    this.callLogs = Array.isArray(logs) ? logs : [];
    ...
    // Initialize real-time Firestore synchronization for Chat & Calls
    this.initFirestoreRealtimeSync();

    // Subscribe to modular domain services for seamless reactive updates
    propertyService.subscribe((props) => {
    ...
    authService.subscribe((user) => {
      if (user && user.id && user.id !== 'user-guest' && user.id !== 'guest' && user.role !== 'guest') {
    ...
    bookingService.subscribe((books) => {
  ...
  public initFirestoreRealtimeSync() {
    if (typeof window === 'undefined') return;
    if (!this.currentUser || !this.currentUser.id || this.currentUser.id === 'guest' || this.currentUser.id === 'user-guest' || this.currentUser.role === 'guest') {
      if (this.firestoreChatUnsub) { this.firestoreChatUnsub(); this.firestoreChatUnsub = null; }
      if (this.firestoreNotifUnsub) { this.firestoreNotifUnsub(); this.firestoreNotifUnsub = null; }
      return;
    }

    try {
      // Sync user profile to Firestore
      firestoreRealtime.syncUserProfile(this.currentUser).catch(() => {});

      if (this.firestoreChatUnsub) { this.firestoreChatUnsub(); }
      if (this.firestoreNotifUnsub) { this.firestoreNotifUnsub(); }
      if (this.firestoreCallLogsUnsub) { this.firestoreCallLogsUnsub(); }

      // 1. Real-time chat rooms listener
      this.firestoreChatUnsub = firestoreRealtime.subscribeToUserChatRooms(
        this.currentUser.id,
        ...
        (cloudRooms) => {
          if (cloudRooms && cloudRooms.length > 0) {
            ...
            cloudRooms.forEach(r => {
              ...
              const localMsgs = this.chatMessages[r.id] || [];
              const allLocalMsgsRead = localMsgs.length > 0 && localMsgs.every(m => m.isRead || m.senderId === this.currentUser.id);
              const effectiveUnread = allLocalMsgsRead ? 0 : (r.unreadCount !== undefined ? r.unreadCount : (existing?.unreadCount || 0));
            ...
            this.chatRooms = Array.from(map.values()).sort((a, b) => (b.id > a.id ? 1 : -1));
      ...
      this.firestoreNotifUnsub = firestoreRealtime.subscribeToUserNotifications(
        this.currentUser.id,
        (cloudNotifs) => {
          if (cloudNotifs) {
            ...
            cloudNotifs.forEach(n => {
              ...
              const isRead = local?.isRead || n.isRead;
              map.set(n.id, { ...n, isRead });
            });
            this.notifications = Array.from(map.values()).sort((a, b) => (b.id > a.id ? 1 : -1));
      ...
      // 3. Real-time Call Logs listener
      this.firestoreCallLogsUnsub = firestoreRealtime.subscribeToUserCallLogs(
        this.currentUser.id,
        (cloudLogs) => {
          if (cloudLogs && cloudLogs.length > 0) {
            const map = new Map<string, CallLog>();
            this.callLogs.forEach(l => map.set(l.id, l));
            cloudLogs.forEach(l => map.set(l.id, l));
            this.callLogs = Array.from(map.values()).sort((a, b) => (b.createdAt > a.createdAt ? 1 : -1));
            setStored(STORAGE_KEYS.CALL_LOGS, this.callLogs);
            this.notify();
          }
        }
      );
    } catch (err) {
      console.warn('Firestore sync setup note:', err);
    }
  }
  ...
  private initSupabaseSync() {
    if (typeof window === 'undefined') return;
    try {
      // 1. Fetch Properties directly from Supabase (Source of Truth)
      supabaseService.getProperties().then(data => {
        if (data && Array.isArray(data) && data.length > 0) {
          const supabaseProps: Property[] = data.map((item: any) => ({
            ...item,
            ownerName: item.owner_name || item.ownerName || 'Verified Lister',
            ...
            userId: item.user_id || item.userId || 'user-lister',
            userRole: item.user_role || item.userRole || 'agent',
            priceFormatted: item.price_formatted || item.priceFormatted || `PKR ${item.price?.toLocaleString() || item.price || 0}`,
            allowOnlineToken: item.allow_online_token ?? item.allowOnlineToken ?? true,
            isPremium: item.is_premium ?? item.isPremium ?? false,
          ...
          // Merge Supabase properties with initial catalog so Supabase items take authoritative precedence
          const supabaseMap = new Map(supabaseProps.map(p => [p.id, p]));
          const merged = [
            ...supabaseProps,
            ...this.properties.filter(p => !supabaseMap.has(p.id))
          ];
      ...
      onAuthStateChanged(auth, async (firebaseUser) => {
        if (firebaseUser) {
          ...
          // Check persistent keyed localStorage for user by ID and email
          let keyedProfile: Partial<User> = {};
          if (typeof window !== 'undefined') {
            try {
              const byId = localStorage.getItem(`dealfast_user_profile_${firebaseUser.uid}`);
              const byEmail = firebaseUser.email ? localStorage.getItem(`dealfast_user_profile_${firebaseUser.email.toLowerCase()}`) : null;
              if (byId) keyedProfile = { ...keyedProfile, ...JSON.parse(byId) };
              if (byEmail) keyedProfile = { ...keyedProfile, ...JSON.parse(byEmail) };
            } catch (e) {}
          }

          // Fetch from Firestore and Supabase in parallel
          let firestoreProfile: Partial<User> = {};
          let supabaseProfile: Partial<User> = {};
          try {
            const [fsProf, sbProf] = await Promise.allSettled([
              firestoreRealtime.getUserProfile(firebaseUser.uid),
              supabaseService.getUserProfile(firebaseUser.uid)
            ]);
            if (fsProf.status === 'fulfilled' && fsProf.value) {
              firestoreProfile = fsProf.value;
            }
            if (sbProf.status === 'fulfilled' && sbProf.value) {
              supabaseProfile = sbProf.value;
            }
          } catch (e) {
            console.warn('Could not read user profile from cloud:', e);
          }

          const calculatedRole: UserRole = isSuperAdmin
            ? 'admin'
            : (supabaseProfile.role || firestoreProfile.role || keyedProfile.role || storedUser?.role || this.currentUser?.role || 'user');

          // Preserve customized avatar if exists; only use Google photoURL as fallback
          const resolvedAvatar =
            keyedProfile.avatar ||
            firestoreProfile.avatar ||
            supabaseProfile.avatar ||
            (storedUser?.avatar && !storedUser.avatar.includes('googleusercontent.com') ? storedUser.avatar : null) ||
            firebaseUser.photoURL ||
            undefined;

          const resolvedCity =
            firestoreProfile.city ||
            supabaseProfile.city ||
            keyedProfile.city ||
            (storedUser?.city && storedUser.city !== 'Islamabad' ? storedUser.city : null) ||
          ...
          const resolvedCnic =
            firestoreProfile.cnic ||
            supabaseProfile.cnic ||
            keyedProfile.cnic ||
            storedUser?.cnic ||
          ...
          const resolvedAgencyName =
            firestoreProfile.agencyName ||
            supabaseProfile.agencyName ||
            keyedProfile.agencyName ||
            storedUser?.agencyName ||
            this.currentUser?.agencyName ||
            '';
          ...
          const realUser: User = {
            ...
            ...firestoreProfile,
            id: firebaseUser.uid,
            name: firestoreProfile.name || supabaseProfile.name || keyedProfile.name || storedUser?.name || firebaseUser.displayName || firebaseUser.email?.split('@')[0] || 'Member',
            email: firebaseUser.email || firestoreProfile.email || supabaseProfile.email || storedUser?.email || '',
            phone: resolvedPhone,
            city: resolvedCity,
            address: firestoreProfile.address || supabaseProfile.address || keyedProfile.address || storedUser?.address || this.currentUser?.address || '',
            cnic: resolvedCnic,
            agencyName: resolvedAgencyName,
            avatar: resolvedAvatar,
            role: calculatedRole,
            kycStatus: firestoreProfile.kycStatus || supabaseProfile.kycStatus || keyedProfile.kycStatus || storedUser?.kycStatus || 'verified',
            createdAt: firestoreProfile.createdAt || supabaseProfile.createdAt || storedUser?.createdAt || new Date().toISOString()
          };
          ...
          if (typeof window !== 'undefined') {
            try {
              localStorage.setItem(`dealfast_user_profile_${firebaseUser.uid}`, JSON.stringify(realUser));
              if (firebaseUser.email) {
                localStorage.setItem(`dealfast_user_profile_${firebaseUser.email.toLowerCase()}`, JSON.stringify(realUser));
              }
            } catch (e) {}
          }
          supabaseService.upsertUserProfile(realUser).catch(() => {});
          firestoreRealtime.syncUserProfile(realUser).catch(() => {});
          this.initFirestoreRealtimeSync();
          this.notify();
        } else {
          // If no active Firebase auth session, preserve valid locally logged-in user session
          const storedUser = getStored<User>(STORAGE_KEYS.CURRENT_USER, null as any);
  ...
  public loginUserSession(user: User) {
    ...
    if (typeof window !== 'undefined' && user.id) {
      try {
        localStorage.setItem(`dealfast_user_profile_${user.id}`, JSON.stringify(user));
        if (user.email) {
          localStorage.setItem(`dealfast_user_profile_${user.email.toLowerCase()}`, JSON.stringify(user));
        }
      } catch (e) {}
    }
  ...
  public async loginWithGoogle() {
    try {
      ...
      if (res.user) {
        // Look up existing profile to preserve custom avatar, CNIC, Phone, City
        let existingProfile: Partial<User> = {};
        if (typeof window !== 'undefined') {
          try {
            const byId = localStorage.getItem(`dealfast_user_profile_${res.user.uid}`);
            const byEmail = res.user.email ? localStorage.getItem(`dealfast_user_profile_${res.user.email.toLowerCase()}`) : null;
            if (byId) existingProfile = { ...existingProfile, ...JSON.parse(byId) };
            if (byEmail) existingProfile = { ...existingProfile, ...JSON.parse(byEmail) };
          } catch (e) {}
        }

        try {
          const fsUser = await firestoreRealtime.getUserProfile(res.user.uid);
          if (fsUser) existingProfile = { ...existingProfile, ...fsUser };
        } catch (e) {}

        const u: User = {
          ...existingProfile,
          id: res.user.uid,
          name: existingProfile.name || res.user.displayName || 'Google Member',
          email: res.user.email || existingProfile.email || '',
          avatar: existingProfile.avatar || res.user.photoURL || undefined,
          phone: existingProfile.phone || '',
          city: existingProfile.city || 'Islamabad',
          cnic: existingProfile.cnic || '',
          role: existingProfile.role || 'user',
          kycStatus: existingProfile.kycStatus || 'verified',
        ...
        if (typeof window !== 'undefined') {
          try {
            localStorage.setItem(`dealfast_user_profile_${u.id}`, JSON.stringify(u));
            if (u.email) {
              localStorage.setItem(`dealfast_user_profile_${u.email.toLowerCase()}`, JSON.stringify(u));
            }
          } catch (e) {}
        }
        authService.setCurrentUser(u);
        await supabaseService.upsertUserProfile(u).catch(() => {});
        await firestoreRealtime.syncUserProfile(u).catch(() => {});
        this.initFirestoreRealtimeSync();
        this.notify();
      }
    } catch (e) {
  ...
  public updateUserProfile(updates: Partial<User>) {
    ...
    if (this.currentUser.id && this.currentUser.id !== 'user-guest' && this.currentUser.id !== 'guest') {
      if (typeof window !== 'undefined') {
        try {
          localStorage.setItem(`dealfast_user_profile_${this.currentUser.id}`, JSON.stringify(this.currentUser));
          if (this.currentUser.email) {
            localStorage.setItem(`dealfast_user_profile_${this.currentUser.email.toLowerCase()}`, JSON.stringify(this.currentUser));
          }
        } catch (e) {}
      }
      supabaseService.upsertUserProfile(this.currentUser).catch(() => {});
  ...
  public recordUniquePropertyView(id: string): { incremented: boolean; views: number } {
    if (!id) return { incremented: false, views: 0 };
    
    // Unique device / session visitor identifier
    let visitorId = '';
    try {
      if (typeof window !== 'undefined') {
        visitorId = localStorage.getItem('dealfast_visitor_uuid') || '';
        if (!visitorId) {
          visitorId = 'df_vis_' + Math.random().toString(36).substring(2, 10) + '_' + Date.now();
          localStorage.setItem('dealfast_visitor_uuid', visitorId);
        }
      }
    } catch (e) {
      visitorId = 'df_vis_session_' + Date.now();
    }

    // Read unique views ledger
    const STORAGE_KEY = 'dealfast_unique_views_ledger';
    ...
    const now = Date.now();
    const lastViewTime = ledger[id] || 0;
    // 12 Hours cooldown window per unique IP / browser visitor
    const COOLDOWN_MS = 12 * 60 * 60 * 1000;

    const propIndex = this.properties.findIndex(p => p.id === id);
    if (propIndex === -1) return { incremented: false, views: 0 };

    const currentProp = this.properties[propIndex];
    let currentViews = Number(currentProp.views) || 0;

    if (now - lastViewTime < COOLDOWN_MS) {
      // Already viewed by this unique visitor within cooldown - do not inflate fake views
      return { incremented: false, views: currentViews };
    }

    // Valid unique view: record timestamp and increment
    ledger[id] = now;
    try {
      if (typeof window !== 'undefined') {
    ...
    const updatedProp = this.properties[propIndex];
    supabaseService.upsertProperty({
      ...updatedProp,
      images: JSON.stringify(updatedProp.images || []),
      features: JSON.stringify(updatedProp.features || [])
    ...
    this.notify();
    return { incremented: true, views: currentViews };
  }

  public incrementViews(id: string) {
    this.recordUniquePropertyView(id);
  }
  ...
  public sendMessage(roomId: string, text: string, mediaUrl?: string, mediaType?: 'image' | 'file'): ChatMessage {
    ...
    try {
      ...
      const recipient = room?.participants.find(p => p.id !== this.currentUser.id);
      
      firestoreRealtime.sendRealtimeMessage(roomId, newMsg, recipient?.id).then(() => {
      ...
      }).catch(err => console.warn('Firestore send message notice:', err));
    } catch (e) {}
  ...
  public getOrCreateRoomWithAgent(agentId: string, agentName: string, propertyId?: string, propertyTitle?: string): ChatRoom {
    const p1 = (this.currentUser.id || 'guest').trim();
    const p2 = (agentId || 'agent').trim();
    ...
    let room = this.chatRooms.find(r => r.id === roomId || r.participants.some(p => p.id === agentId));
    if (!room) {
      room = {
        id: roomId,
        participants: [
    ...
    // Register / sync in Firestore Realtime DB in background
    try {
      firestoreRealtime.getOrCreateChatRoom(
        this.currentUser,
  ...
  public markRoomAsRead(roomId: string): void {
    ...
    let changed = false;
    this.chatRooms = this.chatRooms.map(r => {
      if (r.id === roomId && (r.unreadCount || 0) > 0) {
        changed = true;
    ...
    // Always ensure Firestore knows room and messages are read
    if (this.currentUser.id && this.currentUser.id !== 'guest') {
      firestoreRealtime.markRoomMessagesAsRead(roomId, this.currentUser.id).catch(() => {});
    }
  ...
  public addCallLog(logData: {
    ...
    const logId = `call-log-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
    const now = new Date();
    ...
    const newLog: CallLog = {
      ...
      callerId: logData.callerId,
      callerName: logData.callerName,
      callerAvatar: logData.callerAvatar,
      receiverId: logData.receiverId,
      receiverName: logData.receiverName,
      receiverAvatar: logData.receiverAvatar,
      isVideo: logData.isVideo,
      type: logData.type,
      status: logData.status,
      durationSeconds: logData.durationSeconds || 0,
      timestamp: isToday ? `Today, ${timeFormatted}` : timeFormatted,
      createdAt: now.toISOString()
    };

    // Prepend new call log
    this.callLogs = [newLog, ...this.callLogs.filter(l => l.id !== logId)];
    setStored(STORAGE_KEYS.CALL_LOGS, this.callLogs);

    // Save to Firestore Realtime
    try {
      firestoreRealtime.saveCallLog(newLog).catch(() => {});
    } catch (e) {}

    // Broadcast across tabs
    if (typeof window !== 'undefined') {
      try {
        const bc = new BroadcastChannel('dealfast_realtime_calls');
        bc.postMessage({ type: 'NEW_CALL_LOG', log: newLog });
        bc.close();
      } catch (e) {}
    }
  ...
  public deleteCallLog(logId: string): void {
    this.callLogs = this.callLogs.filter(l => l.id !== logId);
    setStored(STORAGE_KEYS.CALL_LOGS, this.callLogs);
    try {
      firestoreRealtime.deleteCallLog(logId).catch(() => {});
    } catch (e) {}
    this.notify();
  }
  ...
  public markNotificationAsRead(id: string) {
    ...
    setStored(STORAGE_KEYS.NOTIFICATIONS, this.notifications);
    if (id) {
      firestoreRealtime.markNotificationAsRead(id).catch(() => {});
    }
    this.notify();
  }
  ...
  public markAllNotificationsAsRead() {
    const current = Array.isArray(this.notifications) ? this.notifications : [];
    this.notifications = current.map(n => ({ ...n, isRead: true }));
    setStored(STORAGE_KEYS.NOTIFICATIONS, this.notifications);
    if (this.currentUser.id && this.currentUser.id !== 'guest') {
      firestoreRealtime.markAllNotificationsAsRead(this.currentUser.id).catch(() => {});
    }
    this.notify();
  }
  ...
  public clearAllNotifications() {
    ...
    setStored(STORAGE_KEYS.NOTIFICATIONS, this.notifications);
    if (this.currentUser.id && this.currentUser.id !== 'guest') {
      firestoreRealtime.clearUserNotifications(this.currentUser.id).catch(() => {});
    }
    this.notify();
  }
  ...
  public getUserWallet(userId?: string): UserWallet {
    ...
    return {
      ...w,
      availableBalance: Math.round((w.availableBalance || 0) * 100) / 100,
      lockedStake: Math.round((w.lockedStake || 0) * 100) / 100,
      totalEarned: Math.round((w.totalEarned || 0) * 100) / 100,
      totalSpent: Math.round((w.totalSpent || 0) * 100) / 100,
      transactions: Array.isArray(w.transactions) ? w.transactions : []
    };
  }

  // 🔥 FIX BUG #2: Cloud synchronization method
  private async syncWalletToCloud(userId: string, wallet: UserWallet) {
    try {
      ...
      console.warn('Wallet cloud sync notice:', e);
    }
  }

  // 🔥 FIX BUG #2: Enhanced topupWallet with limits, transaction lock & precision
  public topupWallet(amount: number, method: PaymentGateway, refId: string): UserWallet {
    ...
    if (typeof amount !== 'number' || isNaN(amount) || !isFinite(amount) || amount < 100 || amount > 10000000) {
      throw new Error('Invalid deposit amount. Deposit amount must be between PKR 100 and PKR 10,000,000.');
    }
    ...
    // 30-second transaction locking check to prevent accidental double submissions
    const now = Date.now();
    const txLockKey = `${uid}:${cleanRef || 'topup'}`;
    const lastPending = this.pendingTransactions.get(txLockKey);
    if (lastPending && (now - lastPending.timestamp < 30000)) {
      throw new Error('Transaction is currently being processed. Please allow up to 30 seconds before retrying.');
    }
    this.pendingTransactions.set(txLockKey, { amount: validAmount, timestamp: now });

    const wallet = this.getUserWallet(uid);
    const isRaast = method === 'raast';
    const tx: WalletTransaction = {
      ...
      referenceId: cleanRef || (isRaast ? `DF-RAAST-${Math.floor(100000 + Math.random() * 900000)}` : `TOPUP-${Math.floor(100000 + Math.random() * 900000)}`),
      raastRrn: isRaast ? cleanRef : undefined,
      description: isRaast ? `Direct Deposit via State Bank RAAST (0% Fee)` : `Wallet Top-Up via ${method.toUpperCase()}`,
      createdAt: new Date().toLocaleString('en-PK')
    };

    wallet.availableBalance = Math.round((wallet.availableBalance + validAmount) * 100) / 100;
    // Show and store latest 50 transactions to prevent memory overload
    wallet.transactions = [tx, ...wallet.transactions].slice(0, 50);
    this.wallets[uid] = wallet;
    setStored('dealfast_wallets', this.wallets);

    // Sync to Supabase cloud
    this.syncWalletToCloud(uid, wallet);

    this.addNotification({
      userId: uid,
      title: isRaast ? '⚡ Raast Escrow Deposit Successful' : '💳 Wallet Deposit Successful',
      message: isRaast
        ? `PKR ${validAmount.toLocaleString('en-PK', { minimumFractionDigits: 0, maximumFractionDigits: 2 })} deposited with 0% fee via State Bank Raast (Ref: ${tx.referenceId}).`
        : `PKR ${validAmount.toLocaleString('en-PK', { minimumFractionDigits: 0, maximumFractionDigits: 2 })} added to your DealFast Escrow Wallet via ${method.toUpperCase()}.`,
      type: 'system',
      timestamp: 'Just now',
      isRead: false
    });
  ...
  // 🔥 FIX BUG #2: Enhanced withdrawWallet with balance check, daily limit (PKR 500k) & cloud sync
  public withdrawWallet(amount: number, method: PaymentGateway, accountDetails: string): boolean {
    if (this.currentUser.role === 'guest' || this.currentUser.id === 'user-guest') {
      return false;
    }
    if (typeof amount !== 'number' || isNaN(amount) || !isFinite(amount) || amount < 100) {
      return false;
    }
    ...
    // Check available balance minus locked escrow stakes
    const unencumberedBalance = Math.round((wallet.availableBalance - (wallet.lockedStake || 0)) * 100) / 100;
    if (unencumberedBalance < validAmount) {
      return false;
    }

    // Check daily withdrawal limit (PKR 500,000 per user per day)
    const todayStr = new Date().toISOString().split('T')[0];
    const userDaily = this.dailyWithdrawalLimits.get(uid);
    const currentDailyTotal = (userDaily && userDaily.date === todayStr) ? userDaily.total : 0;
    if (currentDailyTotal + validAmount > 500000) {
      throw new Error(`Daily withdrawal limit of PKR 500,000 exceeded. You have withdrawn PKR ${currentDailyTotal.toLocaleString('en-PK')} today. Remaining limit: PKR ${Math.max(0, 500000 - currentDailyTotal).toLocaleString('en-PK')}.`);
    }

    // Update daily withdrawal tracker
    this.dailyWithdrawalLimits.set(uid, {
      date: todayStr,
      total: Math.round((currentDailyTotal + validAmount) * 100) / 100
    });
    ...
    this.addNotification({
      ...
      message: `PKR ${validAmount.toLocaleString('en-PK', { minimumFractionDigits: 0, maximumFractionDigits: 2 })} withdrawn to ${method.toUpperCase()} (${cleanDetails}).`,
      type: 'system',
      timestamp: 'Just now',
      isRead: false
    });
  ...
  public deleteStaffUser(staffId: string): void {
    const target = this.staffUsers.find(s => s.id === staffId);
    const targetName = target ? `${target.name} (@${target.username})` : staffId;
    this.staffUsers = this.staffUsers.filter(s => s.id !== staffId);
    this.staffLoginRequests = this.staffLoginRequests.filter(r => r.staffId !== staffId);
    setStored('dealfast_staff_users', this.staffUsers);
    setStored('dealfast_staff_login_reqs', this.staffLoginRequests);
    this.logSecurityEvent(`Staff Worker Deleted & All Access Revoked: ${targetName}`, 'Super Admin', 'alert');
    this.notify();
  }

  public clearAllStaffUsers(): void {
    const count = this.staffUsers.length;
    this.staffUsers = [];
    this.staffLoginRequests = [];
    setStored('dealfast_staff_users', []);
    setStored('dealfast_staff_login_reqs', []);
    this.logSecurityEvent(`Security Purge: All ${count} Staff Worker Accounts Cleared`, 'Super Admin', 'alert');
    this.notify();
  }
