...
export class AppStore {
      ...
      onAuthStateChanged(auth, async (firebaseUser) => {
        if (firebaseUser) {
          ...
          const realUser: User = {
            ...
            city: supabaseProfile.city || storedUser?.city || this.currentUser?.city || '',
  ...
  public topupWallet(amount: number, method: PaymentGateway, refId: string): UserWallet {
    ...
    this.addNotification({
      ...
      message: isRaast
        ? `PKR ${amount.toLocaleString()} deposited with 0% fee via State Bank Raast (Ref: ${tx.referenceId}).`
        : `PKR ${amount.toLocaleString()} added to your DealFast Escrow Wallet via ${method.toUpperCase()}.`,
      type: 'system',
      timestamp: 'Just now',
      isRead: false
    });
